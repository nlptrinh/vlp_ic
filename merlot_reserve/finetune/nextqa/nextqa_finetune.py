...
# trinh
nextqa_path = '/mnt/disks/persist/merlot_reserve/finetune/nextqa'
nextqa_data_path = '/mnt/disks/persist/merlot_reserve/finetune/datasets/nextqa/tfrecord'
config['data']['train_fns'] = "{}/train{:03d}of{:03d}.tfrecord".format(nextqa_data_path, args.fold, args.num_folds) # input file names in tfrecord format
ckpt = '/mnt/disks/persist/merlot_reserve/model_checkpoints/base_resadapt'
...
config['device']['batch_size'] = 32 # trinh tuning for base/large
config['device']['prefetch_size'] = 0
config['device']['n_fns_per_cycle'] = 1#256 # trinh adjust after creating tfrecord files
...
cfg_name = pretrain_config_file.split('/')[-1]
...
config['device']['output_dir'] = f'{nextqa_path}/nextqa_outputs/{cfg_name}/'
if args.output_name != '':
...
ds_train_iter = finetune_input_fn_builder(config, 'nextqa')
...
class MerlotReservenextqa(MerlotReserve):
    def setup(self):
        super().setup()
...
model = MerlotReservenextqa.from_config(config)
...
def val_epoch(state: train_state.TrainState):
    ...
    # val_config['data']['val_fns'] = "path_to_nextqa/val{:03d}of008.tfrecord"
    val_config['data']['val_fns'] = f"{nextqa_data_path}/val000of064.tfrecord"
    val_config['data']['num_val_files'] = 8
    val_config['data']['do_random_scale'] = False
    val_config['data']['batch_size'] = args.val_batch_size

    val_iter = finetune_val_input_fn_builder(val_config, 'nextqa')
...
for n in range(config['optimizer']['num_train_steps']+1):
    st = time.time()
    # __import__('ipdb').set_trace()
