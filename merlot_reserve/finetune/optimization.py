...
def subtract_old_weights(weight_decay: float = 0.0, mask=None) -> GradientTransformation:
    ...
    def update_fn(updates, state, params=None):
        updates = jax.tree_util.tree_map(lambda g, orig_param: g - weight_decay * orig_param.astype(g.dtype),
...
def finetune_train_step(state: train_state.TrainState, batch,
    ...
    if scan_minibatch:
        ...
        def _microbatch(old_grads, microbatch):
            ...
            grads = jax.tree_util.tree_map(lambda a, b: a + b, old_grads, grads)
    ...
    updates = jax.tree_util.tree_map(_fix_grad, updates, state.params)
